...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        channels_list = [channels * m for m in channel_multipliers]  #64,128,256,256
        # Prepare levels
    ...
    def forward(self, x: torch.Tensor, time_steps: torch.Tensor, cond):
        """
    ...
    def forward(self, x, t_emb, cond=None):
        ...
        for layer in self:
            if isinstance(layer, ResBlock):
                # print("ResBlock input: ",x.shape,t_emb.shape)
                x = layer(x, t_emb)
                # print("ResBlock output: ", x.shape)
            elif isinstance(layer, SpatialTransformer):
                # print("Transformer input: ", x.shape, cond.shape)
                x = layer(x, cond)
...
def _test_time_embeddings():
    ...
    # print(te[:, [50, 100, 190, 260]].numpy().shape)
