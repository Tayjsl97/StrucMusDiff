...
class LatentDiffusion(nn.Module):
    ...
    * [U-Net](model/unet.html) with [attention](model/unet_attention.html)
    ...
    def loss(
        ...
        # print("x0,xt: ",x0.shape,xt.shape)
        # Get $\textcolor{lightgreen}{\epsilon_\theta}(\sqrt{\bar\alpha_t} x_0 + \sqrt{1-\bar\alpha_t}\epsilon, t)$
        # print("eps_model input: ",xt.shape,t.shape,cond.shape)
    ...
    def loss_inpaint(
        ...
        # Get $\textcolor{lightgreen}{\epsilon_\theta}(\sqrt{\bar\alpha_t} x_0 + \sqrt{1-\bar\alpha_t}\epsilon, t)$
        # print("eps_model input: ",xt.shape,t.shape,cond.shape)
        # print("device check: ",xt.device,t.device,cond.device)
        eps_theta = self.eps_model(xt, t, cond)
        # noise_loss = F.mse_loss(noise, eps_theta)
        noise_loss1 = F.mse_loss(noise[:,:,:64,:], eps_theta[:,:,:64,:])
        noise_loss2 = F.mse_loss(noise[:, :,64:, :], eps_theta[:, :,64:, :])

        sqrt_recip_alpha_bar = self.alpha_bar ** -.5
        sqrt_recip_m1_alpha_bar = (1 / self.alpha_bar - 1) ** .5
        # $\frac{1}{\sqrt{\bar\alpha_t}}$
        sqrt_recip_alpha_bar = sqrt_recip_alpha_bar[t].reshape(-1,1,1,1)
        # $\sqrt{\frac{1}{\bar\alpha_t} - 1}$
        sqrt_recip_m1_alpha_bar =sqrt_recip_m1_alpha_bar[t].reshape(-1,1,1,1)
        # print(sqrt_recip_alpha_bar.shape,sqrt_recip_m1_alpha_bar.shape)

        x0_gen = sqrt_recip_alpha_bar * xt - sqrt_recip_m1_alpha_bar * eps_theta
        # print("x0_gen: ",x0_gen.shape)

        # MSE loss
        return noise_loss1,noise_loss2,F.mse_loss(x0[:,:,64:,:], x0_gen[:,:,64:,:]),x0_gen[:,:,64:,:]


    def loss_inpaint_old(
        self,
        x0: torch.Tensor,
        cond: tuple,
        noise: Optional[torch.Tensor] = None,
        ...
        x0_gen = sqrt_recip_alpha_bar * xt - sqrt_recip_m1_alpha_bar * eps_theta
        # MSE loss
        return F.mse_loss(noise, eps_theta),F.mse_loss(x0, x0_gen),x0_gen[:,:,64:,:]
