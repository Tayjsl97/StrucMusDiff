...
class DiffusionSampler:
    ...
    def get_eps(
        ...
        # e_t_uncond = self.model(x, t, uncond_cond)
        # e_t_cond = self.model(x, t, c)
        # Calculate
        # $$\epsilon_\theta(x_t, c) = s\epsilon_\text{cond}(x_t, c) + (s - 1)\epsilon_\text{cond}(x_t, c_u)$$
        # (1+w)\epsilon_\{cond}(x_t, c)-w\epsilon_\{cond}(x_t, u)
        # w=1-w
        # w\epsilon_\{cond}(x_t, c)+(1-w)\epsilon_\{cond}(x_t, u)
        # \epsilon_\{cond}(x_t, u)+w(\epsilon_\{cond}(x_t, c)-\epsilon_\{cond}(x_t, u))
